...
# carry-forward guard fire on unrelated outages: an HDX 403 combined with a
# legitimate drop in scene count looked identical to the scene source degrading,
# and the guard restored a stale scene list over a correct one.
FAILURES = []
...
def fetch_planet_crisis(cfg):
    ...
    for item_url in _walk_stac(root):
        ...
        # One catalogue entry per scene, carrying its available renderings.
        ...
        if "visual" in assets:
            ...
            renders.append({
                ...
                "note": "8-bit RGB as published by the provider.",
            })
        ...
        # that sensor drop the duplicate natural-colour rendering and keep only
        # what adds something.
